...
4. **It restores the 10-14 target honestly** instead of quietly abandoning it.
   0.5.0 becomes 5 verbs plus substantial repair; 0.6.0 lands the themed group
   at 10-13.
...
and any other `nchar()`-on-user-text site the audit turns up.
...
7. **§10 is a catalogue, not a plan.** The audience section exists so ideas stop
   ...
   failure mode is treating it as a backlog to burn down: 0.5.0's cap is 13
   verbs (§3), and §10's only 0.5.0-eligible item is the S-sized
...
and have moved up to Part A (the grapheme fix, the `time` message, and
`emoji_version_profile()`'s grouped guard). What is left below still **fails on
the current tree**, and is the acceptance criteria for §3.1's 0.5.0 correctness
items, in the same order as §1.*
