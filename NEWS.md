...
* Documented that unqualified single-codepoint glyphs present in the sentiment
  lexicon — notably the bare heart `❤` (U+2764, without the `U+FE0F` variation
  selector) — are not detected by the grapheme-aware extractor, so they are not
  scored or counted; supply the emoji-presentation (qualified) form (#3).
