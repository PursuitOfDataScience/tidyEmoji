...
* **232 of the catalogue's 2501 zero-width-joiner sequences were detected as
  ...
  **80.1% to 95.8%** and orphaned joiners from **793 to 2** — the two being
  spellings with no detectable component at all, both of which have a canonical
  form that is found. Well-formed text costs about 6% more; text that was
  broken now costs more and is right.
...
* `text_to_emoji()` no longer claims flatly to be the inverse of
  ...
  and back preserves the key for all 5042 entries and the exact bytes for the
  79% that were already qualified; a second pass changes nothing. The details
  now say so, and say to compare with `emoji_key()` rather than string
  equality.
