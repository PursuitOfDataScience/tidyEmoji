...
# UTF-8 session is a single, different code point, which is 932 pairs beyond
# the Latin tables and brings the whole table to 1158. Greek and Cyrillic and
# their extensions, Armenian, Georgian, Cherokee, Coptic, Glagolitic, Latin
# Extended Additional (which is where Vietnamese lives), Greek Extended,
# Latin Extended-C/D/E, the letterlike and circled forms, and fullwidth Latin.
# A test re-derives the properties the fold needs: no duplicate source, no
# character its own image, no lowercase target that is also an uppercase
# source, and -- in a UTF-8 session only, since that is the only place
# tolower() can be a fair reference -- the table reproduces tolower() exactly.
#
# NOTE: nothing above U+FFFF, and that is a hard constraint rather than an
# oversight. `wchar_t` is 16 bits on Windows, so both chartr() and tolower()
# operate on UTF-16 code units there and mangle a character outside the BMP:
# an earlier version of this table carried the Deseret, Osage, Old Hungarian,
# Warang Citi, Medefaidrin and Adlam pairs, and on windows-latest every one of
# those ranges came back as a single code point repeated. It passed on Linux
# and macOS, where `wchar_t` is 32 bits. 225 astral pairs were dropped; the
# trailing tolower() still handles them in a UTF-8 session exactly as it did
# before the table existed, so nothing regresses. The test asserts the bound.
#
