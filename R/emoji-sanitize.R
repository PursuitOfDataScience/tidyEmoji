...
#'   so `strip` repeats until there is nothing left to remove. It is the one
...
#'   policies can return a column that still holds emoji. `"strip"` never
#'   does. Neither does `"placeholder"`, unless the token you supply is
#'   itself emoji-forming: one that *is* an emoji survives by definition, and
#'   a lone combining character such as `U+FE0F` binds to whatever the
#'   removed glyph was standing next to. `name` is also the accessibility
#'   answer: it is what a screen reader announces.
...
#' to restore.
#'
#' **The default `wrap` also assumes something about the input text, and this is
#' the one failure mode that choosing it does not remove.** [text_to_emoji()]
#' cannot tell a `:shortcode:` token this verb wrote from one the text already
#' held, so a corpus that arrives with literal shortcode tokens in it gains an
#' emoji where it had none. That is not a corner case: it is how people type in
#' the Slack, Discord and GitHub exports the LLM-preprocessing workflow is aimed
#' at.
#'
...
#'   `"nice work :thumbsup:"` followed by `U+1F600` restores with two emoji.
#' * The damage is bounded at one pass. A second round trip changes nothing,
#'   because by then the token is a glyph.
#'
#' The colon hazards [text_to_emoji()] *does* handle are the ones that are not
#' shortcode-shaped: a clock time (`"meet at 10:30"`) and a ratio
#' (`"ratio 3:4"`) both survive untouched, because the token it matches is a
#' colon, alias characters, and a closing colon. So the contract is precise:
#' the round trip restores the original text exactly when the only
#' `:shortcode:`-shaped tokens in it are the ones this verb wrote. If your text
#' may hold real ones, compare `emoji_density()`'s `.emoji_n` before and after,
#' which is the cheapest way to see the inflation.
#'
