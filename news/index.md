...
No new verbs. In a UTF-8 session every verb but one returns exactly what
it returned in 0.4.0: one returns it a great deal faster, and one
answered a non-Latin-script corpus wrongly in a non-UTF-8 locale and now
does not. The rest of the release is documentation and the
infrastructure that checks it.
...
- `spelling` is wired into `tests/spelling.R` (non-failing, skipped on
  ...
  NEWS.md; `inst/WORDLIST` is 170 entries.
