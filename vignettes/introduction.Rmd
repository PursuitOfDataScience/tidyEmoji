...
so emoji carrying the `U+FE0F` variation selector resolve correctly. The lexicon
API is pluggable — see `emoji_lexicons()`, `register_emoji_lexicon()` and the
generic `emoji_score()` to bring your own lexicon.
...
For ad-hoc, vector-level use there are also `as_emoji_name()`,
`as_emoji_shortcode()` and `as_emoji()`, and `emoji_search()` to look up emoji
by keyword:
