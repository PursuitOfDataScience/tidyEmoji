...
```{r, fig.alt = "Bar chart of the number of emoji per emoji-bearing entry. The vast majority of entries contain a single emoji, with a long, thin tail of more emoji-heavy entries."}
emoji_per_tweet %>%
  group_by(.row_number) %>%
...
The overwhelming majority of emoji-bearing entries carry just one emoji, with a
...
```{r, fig.alt = "Horizontal bar chart of the most common emoji category combinations that appear in more than 20 entries."}
ata_emoji_category %>%
...
```{r, fig.alt = "Histogram of the mean emoji sentiment per entry, which is concentrated on the positive side of the scale."}
ata_sentiment %>%
...
```{r, fig.alt = "Horizontal bar chart of the most frequent emoji pairs, labelled by the two glyphs of each pair."}
emoji_edges %>%
  slice_max(n, n = 10) %>%
...
```{r, fig.alt = "Histogram of the mean relative position of emoji within each entry, showing emoji concentrated towards the end of the text."}
ata_tweets %>%
...
Two of the time verbs need no timestamp at all, because the reference table
already records the Unicode version that introduced each glyph. That makes
"how new is this corpus's emoji vocabulary?" a single call:
